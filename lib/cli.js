...
const isColorSupported = () => {
	const { env = {}, argv = [], platform = "", stdout, stderr } = process;

	/**
	 * @param {string | undefined} value value to normalize
	 * @returns {string | undefined} normalized value
	 */
	const lower = (value) =>
		typeof value === "string" ? value.toLowerCase() : value;
	/**
	 * @param {RegExp} regex expression to test
	 * @returns {boolean} true when the matching argument exists
	 */
	const hasArg = (regex) => argv.some((arg) => regex.test(arg));

	const forceColorEnv = lower(env.FORCE_COLOR);
	const isForceColorEnv =
		forceColorEnv !== undefined &&
		forceColorEnv !== "" &&
		forceColorEnv !== "0" &&
		forceColorEnv !== "false";
	const isForceColorFlag = hasArg(
		/^--color(?::|=)?(true|force|always|16m|full|256)?$/i
	);
	const isForced = isForceColorEnv || isForceColorFlag;

	if (isForced) {
		return true;
	}

	const noColorEnv = lower(env.NO_COLOR);
	const isNoColorEnv =
		...
		noColorEnv !== "false";
	const isNoColorFlag =
		hasArg(/^--no-color(?:=(?:true|1))?$/i) || hasArg(/^--color=(?:false|0)$/i);

	if (isNoColorEnv || isNoColorFlag) {
		return false;
	}
	...
	const hasTTY =
		streams.some((stream) => Boolean(stream.isTTY)) ||
		(typeof tty.isatty === "function" && tty.isatty(1));
	const isWindows = platform === "win32";
	const term = env.TERM || "";
	const isDumbTerminal = term === "dumb";

	if (hasTTY && term && !isDumbTerminal) {
		return true;
	}

	if (isWindows && !isDumbTerminal) {
		return true;
	}
	...
	// Fall back to enabling colors when nothing explicitly disables them.
	return true;
};
