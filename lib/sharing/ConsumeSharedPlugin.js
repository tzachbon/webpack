				...
				compilation.hooks.finishModules.tapAsync(
					...
					(modules, callback) => {
						for (const module of modules) {
							...
							const { buildMeta: consumedMeta, buildInfo: consumedInfo } =
								fallbackModule;
							if (consumedMeta) {
								const meta = (module.buildMeta = { ...module.buildMeta });
								if (consumedMeta.defaultObject !== undefined) {
									meta.defaultObject = consumedMeta.defaultObject;
								}
							}

							if (consumedInfo) {
								const info = (module.buildInfo = { ...module.buildInfo });
								if (consumedInfo.strict !== undefined) {
									info.strict = consumedInfo.strict;
								}
								if (consumedInfo.exportsArgument !== undefined) {
									info.exportsArgument = consumedInfo.exportsArgument;
								}
								if (consumedInfo.moduleArgument !== undefined) {
									info.moduleArgument = consumedInfo.moduleArgument;
								}
								if (consumedInfo.topLevelDeclarations !== undefined) {
									info.topLevelDeclarations = new Set(
										consumedInfo.topLevelDeclarations
									);
								}
							}
						}
